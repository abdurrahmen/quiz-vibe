...
import type { Question, DuelProgress, Duel } from '@/lib/types'
import OpponentProgressBar from './OpponentProgressBar'

interface CategoryWarsBattleProps {
  duel: Duel
  questions: Question[]
  myUsername: string
  opponentName: string
...
export default function CategoryWarsBattle({
  duel,
  questions,
  myUsername,
  opponentName,
  ...
  // Determine turf for each question index
  // Even indices (0,2,4,6,8) = creator's turf, Odd (1,3,5,7,9) = opponent's turf
  const isMyTurf = (idx: number) => {
    if (duel.mode !== 'category_wars') return true
    const isCreatorQuestion = idx % 2 === 0
    const iAmCreator = duel.creator_name === myUsername
    return iAmCreator ? isCreatorQuestion : !isCreatorQuestion
  }
  ...
  const turf = isMyTurf(currentIndex)

  const getOptionClass = (idx: number) => {
    if (!answered) return 'border-surface-variant bg-white text-on-surface hover:border-primary hover:bg-primary/5 cursor-pointer'
    if (q.correct_answers.includes(idx)) return 'border-green-500 bg-green-50 text-green-800 font-bold'
    if (idx === selectedOption) return 'border-error bg-error-container/20 text-error'
  ...
  return (
    <div className="min-h-screen bg-surface-container-lowest flex flex-col">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          key={`turf-${currentIndex}`}
          initial={{ opacity: 0, y: -8 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0 }}
          className={`px-4 py-2.5 flex items-center justify-center gap-2 text-sm font-black uppercase tracking-widest ${
            turf
              ? 'bg-primary/10 text-primary border-b-2 border-primary/20'
              : 'bg-error/10 text-error border-b-2 border-error/20'
          }`}
        >
          <span className="text-base">{turf ? '🏠' : '⚔️'}</span>
          {turf ? 'Your Turf — Home Advantage!' : "Opponent's Turf — Fight for it!"}
        </motion.div>
      </AnimatePresence>
      ...
      <div className="flex-1 flex items-center justify-center p-4 md:p-8">
        <div className="w-full max-w-2xl">
          ...
          <div className="flex justify-center gap-1.5 mb-8">
            {questions.map((_, i) => {
              const myT = isMyTurf(i)
              return (
                <div
                  key={i}
                  className={`h-1.5 rounded-full transition-all duration-300 ${
                    i < currentIndex
                      ? myT ? 'w-6 bg-primary' : 'w-6 bg-error/70'
                      : i === currentIndex
                      ? myT ? 'w-8 bg-primary' : 'w-8 bg-error'
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {/* Question card — border color reflects turf */}
              <div className={`bg-white rounded-2xl shadow-ambient border-2 p-6 md:p-8 mb-6 ${
                turf ? 'border-primary/20' : 'border-error/20'
              }`}>
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-black text-sm shrink-0 ${
                    turf ? 'bg-primary' : 'bg-error'
                  }`}>
                    {currentIndex + 1}
                  </div>
